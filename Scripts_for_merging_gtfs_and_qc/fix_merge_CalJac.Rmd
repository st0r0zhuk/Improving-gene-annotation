---
title: "Add isoforms as gene models to reference gtf"
output: html_document
date: "2025-08-13"
---

# convert reference gtf to gff
```{bash}
mamba activate agat_env

# turn reference into gff
agat_convert_sp_gxf2gxf.pl \
  --gff /work/vstorozhuk/data/reference/GCF_011100555.1_mCalJa1.2.pat.X_genomic.standardized.gtf \
  -o ref.std.gff3
  
```

# extract selected MANDALORION isoforms from gff after sqanti qc
```{r}
# load previously collected table with all isoforms that passed filter and have suppot of at least 2 reads
all_tables <- read.csv("/work/vstorozhuk/export_to_git/Scripts_for_analysis_of_isoforms/classifications_support2.txt")
all_tables %<>% filter(structural_category != "fusion") # do not take fusions, not informative

# table also already contains added metadata on CAGE-peaks and number of long-reads aligned
barcode01_manda <- all_tables %>% 
  filter(Species == "Callithrix jacchus", pipeline == "MANDALORION") %>% 
  dplyr::select(isoform, structural_category, subcategory, Isoform_N_reads, within_CAGE_peak, dist_to_CAGE_peak)

# load isoforms with CDS and exon ranges
gtf_manda <- fread("/work/vstorozhuk/data/iso_pipeline/pipeline_outputs/Barcode04/Barcode04_mandalorion_R2_sqanti_report_postfilter/isoforms_corrected.cds.gff3",
            sep = "\t",
            header = FALSE,
            quote = "",
            fill = TRUE)
...
gtf_mandalorion_filtered <- gtf_manda %>%
  ...
  #if gene is novel, specifically call it Novel_ + internal id (e.g. Novel_1)
    gene_id = if_else(
  ...
  # filter to only the isoforms you care about
  filter(transcript_id %in% barcode01_manda$isoform) %>%
  # join in all the extra stats columns
  left_join(barcode01_manda, by = c("transcript_id" = "isoform")) %>%
  # explicitly tack each one onto V9
  mutate(
    V9 = paste0(
      V9,
...
write.table(
  ...
  file = "/work/vstorozhuk/export_to_git/Scripts_for_merging_gtfs_and_qc/CalJac_intermediate/gtf_mandalorion_filtered.gtf",
  sep = "\t",
  quote = FALSE,
  col.names = FALSE,
  row.names = FALSE
...
# turn selected isoforms into gff, add start-stop codons when possible
```{bash}
mamba activate agat_env
...
agat_sp_add_start_and_stop.pl \
  --gff lr.std.gff3 \
  --fasta /work/vstorozhuk/final_references/GCF_011100555.1_mCalJa1.2.pat.X_genomic.fna \
  -o lr.std_with_utrs_codons.gff3
  
```

74 CDS do not start by a start codon
79 CDS do not end by a stop codon 

# find cases when transcript does not start with start codon
...
isoforms <- rtracklayer::readGFF("/work/vstorozhuk/export_to_git/Scripts_for_merging_gtfs_and_qc/CalJac_intermediate/lr.std_with_utrs_codons.gff3")
...
referece <- rtracklayer::readGFF("/work/vstorozhuk/export_to_git/Scripts_for_merging_gtfs_and_qc/CalJac_intermediate/ref.std.gff3")
...
# Overlap to find if reference start codon overlaps the transcript 
# this way, i specifically do not try to extend the isoforms, only to shorten CDS when it is possible
...
fix_with_start_codon <- function(filtered_isoforms, overlap_df) {
  ...
  # helper to coerce Parent (and a few others) to character
  ...
  utr5 <- x %>%
    ...
    select(names(filtered_isoforms)) %>%
    normalize_cols()
...
# run
df <- fix_with_start_codon(filtered_isoforms, overlap_df)
...
# Write GTF
export(gr, "/work/vstorozhuk/export_to_git/Scripts_for_merging_gtfs_and_qc/CalJac_intermediate/fixed_start_codons.gff", format = "gtf")
...
################## swap models which could be fixed

all_new <- rtracklayer::readGFF("/work/vstorozhuk/export_to_git/Scripts_for_merging_gtfs_and_qc/CalJac_intermediate/lr.std_with_utrs_codons.gff3")
all_new %<>% as.data.frame()

fixed_new <- rtracklayer::readGFF("/work/vstorozhuk/export_to_git/Scripts_for_merging_gtfs_and_qc/CalJac_intermediate/fixed_start_codons.gff")
...
# Write GTF
export(gr, "/work/vstorozhuk/export_to_git/Scripts_for_merging_gtfs_and_qc/CalJac_intermediate/all_new_with_fixed.gff", format = "gtf")
...
# to gff (also adds utrs, and other fixes)
agat_convert_sp_gxf2gxf.pl \
...
isoforms <- rtracklayer::readGFF("/work/vstorozhuk/export_to_git/Scripts_for_merging_gtfs_and_qc/CalJac_intermediate/test.gff3")
isoforms %<>% as.data.frame()
...
mcols(gr) <- final %>%
  ...
  mutate(across(everything(), ~ if (is.list(.)) vapply(., toString, "") else .))
# Write GTF
export(gr, "/work/vstorozhuk/export_to_git/Scripts_for_merging_gtfs_and_qc/CalJac_intermediate/final.gff", format = "gtf")
...
fix cases when parent is wrong, 
remove bistronic isoform
remove LOC which are fixed with new gene
fix misannotated gene
determine dominant transcript per gene
handle UTRs when start or stop codon is missing:
  for cases when start codon could not be recovered - remove 5 prime UTR entry for that transcript
  for cases when end codon is missing - remove 3 prime UTR entry for that transcript
...
merged <- rtracklayer::readGFF("/work/vstorozhuk/export_to_git/Scripts_for_merging_gtfs_and_qc/CalJac_intermediate/merged.with_isoforms.gtf")
merged %<>% as.data.frame()
to_fix <- merged

# remove troubling bicistronic transcripts SNRPN-SNURF and Isoform599_2 which is impossible to curate 
to_fix %<>% dplyr::filter(!(gene_id %in% c("SNRPN", "SNURF") & source == "MANDALORION"))
to_fix %<>% dplyr::filter(!(transcript_id == "Isoform599_2" & source == "MANDALORION"))

# remove loci which become fixed with new gene
to_fix %<>% dplyr::filter(!(gene_id %in% c("LOC128930468", "LOC118146204", "LOC128932359")))

# remove novelGene_1, it is shorter duplicate of novelGene_2
to_fix %<>% dplyr::filter(!(gene_id == "novelGene_1" & source == "MANDALORION"))

# one specific IER3IP1 transcript wrongly becomes LOC103787656 -> rename
to_fix <- to_fix %>%
  mutate(
    gene_id = ifelse(
      gene_id == "LOC103787656" & source == "MANDALORION",
      "IER3IP1",
      gene_id
    ),
    Parent = ifelse(
      gene_id == "IER3IP1" & source == "MANDALORION" & type == "transcript",
      "IER3IP1",
      Parent
    )
  )

# rename old STOML2 XM_054255229.1 to misannotated_STOML2

to_fix <- to_fix %>%
  mutate(
    gene_id = ifelse(gene_id == "STOML2", "misannotated_STOML2", gene_id),
    gene    = ifelse(gene == "STOML2", "misannotated_STOML2", gene),
    Parent  = ifelse(Parent == "STOML2", "misannotated_STOML2", Parent)
  )


# rename novelGene_LOC128932359_AS to STOML2

to_fix <- to_fix %>%
  mutate(
    gene_id = ifelse(gene_id == "novelGene_LOC128932359_AS", "STOML2", gene_id),
    gene    = ifelse(gene == "novelGene_LOC128932359_AS", "STOML2", gene),
    Parent  = ifelse(Parent == "novelGene_LOC128932359_AS", "STOML2", Parent)
  )
...
# Split: keep "other" rows intact, process only MANDALORION non-gene ---
mand <- to_fix %>%
...
# Per-transcript flags for presence of start/stop codons ---
tx_flags <- mand %>%
...
# Join flags back and drop UTRs when codon is missing ---
mand_fixed <- mand %>%
...
# Write GTF
export(gr, "/work/vstorozhuk/export_to_git/Scripts_for_merging_gtfs_and_qc/CalJac_intermediate/mCalJac1.2_extended.gtf", format = "gtf")
...
```{r}
gtf_produced <- rtracklayer::readGFF("/work/vstorozhuk/export_to_git/Scripts_for_merging_gtfs_and_qc/CalJac_intermediate/mCalJac1.2_extended.gtf")
