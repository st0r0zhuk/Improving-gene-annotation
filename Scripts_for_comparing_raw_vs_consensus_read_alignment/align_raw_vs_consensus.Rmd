...
```{bash}
conda env export --no-builds --name pychopper > /work/vstorozhuk/export_to_git/Scripts_for_comparing_raw_vs_consensus_read_alignment/pychopper.yaml
conda env export --no-builds --name c3poa > /work/vstorozhuk/export_to_git/Scripts_for_comparing_raw_vs_consensus_read_alignment/seqtk.yaml
```
...
```
	...
	•	All subreads coming from the same original concatemer will share the same UUID
	take only 2nd subread
	seqtk subseq R2C2_Subreads.fastq <(grep "^@" R2C2_Subreads.fastq | grep "_0" | sed 's/@//') > R2C2_firstSubread.fastq
...
p <- ggplot(df_long, aes(x = dataset, y = pct, fill = component)) +
  ...
  geom_text(
    ...
    color = "white",                         # readable on colored bars
    size = 3
  ) +
...
# index sam, turn into bam and get stats
```{bash}
conda activate gffcompare
...
# ---------------------------
# 2) Helper: extract error rate from summary.txt
# ---------------------------
...
# keep your factor order as you set it
...
ggplot(df, aes(x = species, y = error_rate, fill = dataset)) +
  ...
  scale_fill_manual(values = c(
    "Second subread" = "#F44336",  # red
    "Consensus"      = "#4C9F70"   # green
  )) +
