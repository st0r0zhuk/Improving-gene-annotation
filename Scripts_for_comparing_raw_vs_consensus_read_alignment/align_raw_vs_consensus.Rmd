...
df <- df %>%
  mutate(
    species = recode(species, !!!species_lut),
    species = factor(species,
      levels = c("Macaca mulatta",
                 "Homo sapiens",
                 "Pan troglodytes",
                 "Callithrix jacchus")),
    dataset = factor(dataset,
      levels = c("Consensus", "Second subread"))
  )
...
p <- ggplot(df_long, aes(x = dataset, y = pct, fill = component)) +
  ...
  scale_fill_manual(values = c("Two primers" = "#4C9F70",
                               "Rescued"     = "#84C999",
                               "Unusable"    = "#F44336")) +
  labs(x = NULL, y = "Percent of reads", fill = NULL)  +  theme_minimal(base_size = 12) +
  theme(
...
ggsave("/work/vstorozhuk/data/plots/trimming_rate.png", p, dpi = 600,width = 10,height = 6)
...
error_rate <- ggplot(df, aes(x = species, y = error_rate, fill = dataset)) +
...
ggsave("/work/vstorozhuk/data/plots/mapping_error_rate.png", error_rate, dpi = 600,width = 14,height = 6)
